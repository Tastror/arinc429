...
unsigned short Get429ConfigureWord(unsigned short baudrate,unsigned short parity, unsigned short workmode)
{
	...
	switch (baudrate) // configure baudrate 设置传输速率
	{
		case C429_BITRATE100K:
			cw = cw & 0x9ff0;//0x9ff0数据接口
			break;
	...
	switch (parity) // configure parity 设置校验位
	{
...
unsigned long Resume429Word(unsigned long dw429w)    //429字转化计算机字
{
	...
	// bit 8 == 0
	if ((tp&0x00000100)==0x00000100)   //奇偶校验位 
	{
		bit8IsZero = 0;               
	}
	else
